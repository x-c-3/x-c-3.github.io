...
There is a filter on all messages that blocks these characters: `.>{}`, and a payload that works is

```html
<img src=x onerror="javascript:ghurwiejiowrobw" 
```

...as due to the trailing space, any subsequent HTML is then treated either as part of a new attribute or as the closing part of the tag. Submitting this payload with some javascript to exfiltrate the admin's cookie then logging in as the admin yields the flag.
