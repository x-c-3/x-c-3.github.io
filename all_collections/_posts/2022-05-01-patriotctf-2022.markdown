---
layout: post
title:  "PatriotCTF Writeup Collection"
author: "PatriotCTF"
...
...leading to us being able to login if and only if `c` matches with the character in the password at index `pos`. Here's the script.
...
After you get the password, submit another request but replace the brute-forcing password query with your output to get the flag.
...
There is a filter on all messages that blocks these characters: `.>{}`, and a payload that works is `<img src=x onerror="javascript:ghurwiejiowrobw" `, as due to the trailing space, any subsequent HTML is then treated either as part of a new attribute or as the closing part of the tag. Submitting this payload with some javascript to exfiltrate the admin's cookie then logging in as the admin yields the flag.
...
while True:
    ...
    if r.split("Just sent your message: ")[1][0] != "<": # if "works" is output
        i += 1
        print(str(bytes.fromhex(c[2:]))[2:-1], end = "")
        c_i = 0
    else:
        c_i += 1
```
